...
def create_groundtruth_database(dataset_class_name,
    ...
    print(f'Path of the data is {data_path}')
    print(f'Path of the info file is {info_prefix}')
    dataset_cfg = dict(
    ...
    if database_save_path is None:
        database_save_path = osp.join(output_path, f'{info_prefix}_gt_database')
    if db_info_save_path is None:
        db_info_save_path = osp.join(output_path,
                                     f'{info_prefix}_dbinfos_train.pkl')
    mmcv.mkdir_or_exist(database_save_path)
    all_db_infos = dict()
    if with_mask:
        coco = COCO(osp.join(output_path, mask_anno_path))
