...
from mmdet3d.core.utils import draw_umich_gaussian_2D
...
from mmdet3d.models.utils import (filter_outside_objs, get_edge_indices,
                                  get_keypoints, get_flex_keypoints, handle_proj_objs)
...
class MonoFlexHead(AnchorFreeMono3DHead):
    ...
    .. code-block:: none
                ...
                / --> 3 x 3 conv --> 1 x 1 conv --> [edge fusion] --> cls 3
                |
                | --> 3 x 3 conv --> 1 x 1 conv --> 2d bbox 4
                |
                | --> 3 x 3 conv --> 1 x 1 conv --> [edge fusion] --> 2d offsets 2
                |
                | --> 3 x 3 conv --> 1 x 1 conv -->  keypoints offsets 20
                |
                | --> 3 x 3 conv --> 1 x 1 conv -->  keypoints uncertainty 3
        feature
                |
                | --> 3 x 3 conv --> 1 x 1 conv -->   3d dimensions 3 
                |
                |                  |--- 1 x 1 conv -->  ori cls 8
                | --> 3 x 3 conv --|
                |                  |--- 1 x 1 conv -->  ori offsets 8
                |
                | --> 3 x 3 conv --> 1 x 1 conv -->  depth 1
                |
                \ --> 3 x 3 conv --> 1 x 1 conv -->  depth uncertainty 1
    ...
    def forward_train(self, x, input_metas, gt_bboxes, gt_labels, gt_bboxes_3d,
        ...
        """
        Args:
            x (list[Tensor]): Features from FPN 8 x 64 x 96 x 320.
    ...
    def forward_single(self, x, input_metas):
        ...
        Args:
            x (Tensor): Feature maps from a specific FPN feature level. b x 64 x 96 x 320
            input_metas (list[dict]): Meta information of each image, e.g.,
                image size, scaling factor, etc.

        Returns:
            tuple: Scores for each class, bbox predictions.
            cls_score: b x 3 x 96 x 320
            bbox_pred: b x 50 x 96 x 320
        """
    ...
    def decode_heatmap(self,
        ...
        *batch_dets, topk_ys, topk_xs = get_topk_from_heatmap(
        ...
        regression = regression.view(-1, 50) # (b x k) x 50
        pred_base_centers2d = torch.cat(
            [topk_xs.view(-1, 1),
             topk_ys.view(-1, 1).float()], dim=1) # k x 2

        preds = self.bbox_coder.decode(regression, pred_base_centers2d, batch_topk_labels.view(-1),
        ...
        pred_yaws, _ = self.bbox_coder.decode_orientation(
            preds['orientations'], pred_locations)
        pred_dims = preds['dimensions']

        batch_bboxes = torch.cat((pred_locations, pred_dims, pred_yaws.unsqueeze(-1)), dim=1)
    ...
    def get_predictions(self, pred_reg, labels3d, centers2d, reg_mask,
        ...
        centers2d_inds = centers2d_inds.view(batch, -1).type(torch.int64)
        pred_regression = transpose_and_gather_feat(pred_reg, centers2d_inds) 
        pred_regression_pois = pred_regression.view(-1, channel)[reg_mask] # N x 50

        centers2d = centers2d.view(-1, 2)[reg_mask] # N x 2
        preds = self.bbox_coder.decode(pred_regression_pois, centers2d, labels3d, downsample_ratio, cam2imgs)
        return preds
...
``
        ...
        # heatmap generation.
        # centers2d = (int(base_centers2d) + offsets2d) * 4
        ...
        keypoints2d_list, keypoints_mask_list, keypoints_depth_mask_list = \
            get_flex_keypoints(gt_bboxes_3d_list, centers2d_list, input_metas)
        ...
        for batch_id in range(batch_size):
            ...
            gt_base_centers2d = base_centers2d_list[batch_id]
        ...
        gt_bboxes_3d = input_metas[0]['box_type_3d'].cat(gt_bboxes_3d_list)
        ...
        batch_base_centers2d = batch_base_centers2d.view(-1, 2)
        ...
        target_labels = dict(
            ...
            keypoints_mask=keypoints_mask.bool(),
            keypoints_depth_mask=keypoints_depth_mask.bool(),
    ...
    def loss(self,
        ...
        keypoints2d_mask = target_labels['keypoints_mask']
        loss_keypoints = self.loss_keypoints(
            preds['keypoints2d'][keypoints2d_mask].to(center2d_heatmap.device),
            target_labels['keypoints2d_target'][keypoints2d_mask].to(center2d_heatmap.device))

        # orientations loss
        loss_dir = self.loss_dir(preds['orientations'],
                                 target_labels['orienations_target'], num_dir_bins=4)
        ...
        loss_keypoints_depth = self.loss_keypoints_depth(
