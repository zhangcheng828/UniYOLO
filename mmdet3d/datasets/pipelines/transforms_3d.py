...
from mmdet3d.core.bbox import box_np_ops, CameraInstance3DBoxes, points_cam2img
...
class AddKeypoints3D(object):
    ...
    def __call__(self, results):
        ...
        #     keypoints_x_visible = (keypoints2d[..., 0] >= results['ori_hw'][2]) & (
        #         keypoints2d[..., 0] <= results['ori_hw'][3] - 1)
        #     keypoints_y_visible = (keypoints2d[..., 1] >= results['ori_hw'][0]) & (
        #         keypoints2d[..., 1] <= results['ori_hw'][1] - 1)
        # else:
        ...
        if results.get('ori_hw', None):
            keypoints_visible = torch.zeros_like(keypoints_visible).bool()
        results['gt_kpts_2d'] = keypoints2d
        results['gt_kpts_valid_mask'] = keypoints_visible
        return results

@PIPELINES.register_module()
class FilterAnnotations3D(object):
    def pts2Dto3D(self, points, view):
        ...
        points2D = points[:, :2]
        depths = points[:, 2:3]
        unnorm_points2D = np.concatenate([points2D * depths, depths], axis=1)

        viewpad = np.eye(4, dtype=points2D.dtype)
        viewpad[:view.shape[0], :view.shape[1]] = view
        inv_viewpad = np.linalg.inv(viewpad).T

        # Do operation in homogenous coordinates.
        nbr_points = unnorm_points2D.shape[0]
        homo_points2D = np.concatenate(
            [unnorm_points2D,
             np.ones((nbr_points, 1))], axis=1)
        points3D = np.matmul(homo_points2D, inv_viewpad)[:, :3]
    ...
    def __call__(self, results):
        ...
        depths = results['depths'].copy()
        valid_inds = (centers2d[:, 0] >
                           0) & (centers2d[:, 0] < w) & (centers2d[:, 1] > 0) & (
                                     centers2d[:, 1] < h) & (depths < 70)
        
        pts = np.concatenate([centers2d, depths.reshape(-1, 1)], axis=1)
        pts = self.pts2Dto3D(pts, cam2img)

        box_3d_tensor = results['gt_bboxes_3d'].tensor # bottom center
        box_3d_tensor[..., :3] = torch.from_numpy(pts) # gravity center

        box_3d_tensor = box_3d_tensor[valid_inds]
        box_3d_tensor = CameraInstance3DBoxes(box_3d_tensor, box_dim=7, origin=(0.5, 0.5, 0.5))
        ...
        results['centers2d'] = results['centers2d'][valid_inds]
            ...
            for key in results.get('bbox_fields', []):
                ...
                if key == 'gt_bboxes':
                    ...
                    if 'gt_bboxes_3d' in results:
                        ...
                        results['centers2d'] = centers2d[valid_inds]
                        results['depths'] = results['depths'][valid_inds]
...
class AffineResize(object):
    ...
    def __init__(self, img_scale, bbox_clip_border=True):
    ...
    def __call__(self, results):
        ...
        results['trans_mat'] = trans_affine
...
class HorizontalShift(object):
    ...
    def __init__(self, factor_range=[0.5, 1.0], bbox_clip_border=True):
    ...
    def __call__(self, results):
        img = results['img']
        scale_factor = random.randint(self.factor_range[0] * 10, self.factor_range[1] * 10 + 1) * 0.1

        h, w = img.shape[:2]
        box_h, box_w = int(h * scale_factor), int(w * scale_factor)
        if box_h == h: return results

        if box_h > h:
            new_img = np.zeros([box_h, box_w, img.shape[2]])
            start_h, start_w = (box_h - h) // 2, (box_w - w) // 2
            end_h, end_w = start_h + h, start_w + w
            new_img[start_h : end_h, start_w : end_w, :] = img
            img = new_img
        else:
            start_h, start_w = (h - box_h) // 2, (w - box_w) // 2
            end_h, end_w = start_h + box_h, start_w + box_w
            img = img[start_h : end_h, start_w : end_w, :]
        img = cv2.resize(img, (w, h))
        results['img'] = img

        start = np.array([start_w, start_h])
        self._scale_bboxes(results, start, scale_factor, h, w)
        if 'centers2d' in results:
            centers2d = self._scale_pos(results['centers2d'], start,
                                               scale_factor)
            valid_index = (centers2d[:, 0] >
                           0) & (centers2d[:, 0] < w) & (centers2d[:, 1] > 0) & (
                                     centers2d[:, 1] < h)
            ...
            results['depths'] = results['depths'][valid_index] * scale_factor

        return results

    def _scale_pos(self, pos, start, scale_factor):
        if scale_factor > 1:
            final_pos = (pos + start) / scale_factor
        else:
            final_pos = (pos - start) / scale_factor
        return final_pos

    def _scale_bboxes(self, results, start, scale_factor, h, w):

        for key in results.get('bbox_fields', []):
            bboxes = results[key]
            diff = bboxes[:, 2:] - bboxes[:, :2]
            bboxes[:, :2] = self._scale_pos(bboxes[:, :2], start, scale_factor)
            
            bboxes[:, 2:] = bboxes[:, :2] + diff / scale_factor
            if self.bbox_clip_border:
                bboxes[:,
                       [0, 2]] = bboxes[:,
                                        [0, 2]].clip(0, w - 1)
