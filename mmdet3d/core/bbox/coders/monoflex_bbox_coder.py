...
class MonoFlexCoder(BaseBBoxCoder):
    ...
    def decode_location(self,
        ...
        Return:
            tuple(torch.Tensor): Centers of 3D boxes.
                shape: (N, 3)
        """
        N = base_centers2d.shape[0]
        if pad_mode == 'default':
            centers2d_img = (base_centers2d + offsets2d) * 4
        else:
            raise NotImplementedError

        depths = depths.view(-1, 1)
        unnorm_points = torch.cat([(centers2d_img * depths), depths], dim=-1)     # (N, 3)

        cam2imgs_inv = cam2imgs.inverse()

        # expand project points as [N, 3, 1]
        unnorm_points = unnorm_points.unsqueeze(-1)

        if cam2imgs.shape[1] == 4:
            unnorm_points = torch.cat(
                (unnorm_points, unnorm_points.new_ones(N, 1, 1)), dim=1)
        locations = torch.matmul(cam2imgs_inv, unnorm_points).squeeze(2)
    ...
    def keypoints2depth(self,
                        ...
                        cam2imgs,
                        downsample_ratio=4,
                        group0_index=[(6, 3), (5, 0)],
                        group1_index=[(2, 7), (1, 4)]):
